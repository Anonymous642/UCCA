...
uint16_t ucc1min __attribute__((section (".ucc1min"))) = 0xE1E8;
uint16_t ucc1max __attribute__((section (".ucc1max"))) = 0xE24E;
uint16_t ucc2min __attribute__((section (".ucc2min"))) = 0xE250;
uint16_t ucc2max __attribute__((section (".ucc2max"))) = 0xE2BE;
...
__attribute__ ((section (".regionOne.body"))) void getUserInput(char* output, char *input){
    ...
    // Jumps to the return instruction before the stack has been properly cleaned up
    __asm__ volatile("br #0xe24E" "\n\t"); 
...
int main(void)
{
	 ...
	 //A buffer that when combined with the malcous jump on line 60 leads to a malicous return
	 char input[6] = {0xB0, 0xE0, 'C', 'D', 'E', '\0'};
